...
##### 7.4. Gold Layer
The Gold layer is built using Spark SQL and serves as an analytical model designed to answer specific business questions. Below you have an example of Gold Layer Table that shows TOP used hashtags for each user which gives advantage in better advertisements targeting.
```sql
...
Just like Workflow Job, a SDP also uses the JDBC interface to igest data from Azure Database, here I'm also addig new column ingest_time, which holds timestamp of when the data was extracted from the source system.
...
#### 8.2 SDP Silver Layer
The Silver Layer (same as Workflow Job approach) is based on implementation of the **SCD Type II** using **apply_changes** method. In the Silver Layer I create temporary tables, here's the example of silver_account_user_clean, additionally I use **Expectations**, when data does not meet the defined criteria, these cases can trigger different pipeline behaviors like dropping, flagging or even stopping the pipiline.
```python
@dlt.table(
...
#### 8.3 SDP Gold Layer
In SDP I use the same tables in Golden Layer. Below is an example of an aggregate table taht stores information about all posts in app. Including the language of post, total number of reactions, comment count, hashtag count, etc.
...
### 9. MLFlow Model
Business Goal: Common challenge for social media apps is the high number of bot accounts. Understanding how to identify such accounts becomes crucial. By accurate flagging "real" users and bots you can enhance credibility among advertisers.

During model training I defined explanatory variables and target variable, split the data into training and test sets, impute missing values with the median, perform feature standarization, and select the best fitting model with optimized hyperparameters.
